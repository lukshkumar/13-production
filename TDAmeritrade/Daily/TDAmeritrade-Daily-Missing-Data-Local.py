...
for entry in entries:
    ...
    if(entry in files_in_data_by_ticker_folder):
        df_complete = pd.read_csv("TDAmeritrade-Daily-Data-By-Ticker-Split-Applied/" + entry)
        df_complete = df_complete.append(df)
        df_complete = df_complete.sort_values(['Date'])
    else:
        df_complete = df.copy()
    df_complete.to_csv("TDAmeritrade-Daily-Data-By-Ticker-Split-Applied/" + entry, index = False, columns = ['Date', 'open','high','low','close','volume'])
