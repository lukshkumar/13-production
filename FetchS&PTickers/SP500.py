...
def get_sp500_symbols_from_wikipedia():
    """
    Get all S&P 500 component stocks from wikipedia by scraping the webpage.
    """
...
def get_sp500_symbols_from_financialmodelingprep():
    """
    Get all S&P 500 component stocks from financial modeling prep API
    """

    endpoint = "https://financialmodelingprep.com/api/v3/sp500_constituent?apikey=aa8b4631b2ad1b2704741d1a2d6a2611"
    json_data = r.get(endpoint)
    ...
    #Saving each day SNP 500 List into snp folder as a csv file.
    df_snp_500 = pd.DataFrame(stocks, columns = ['Symbol'])
    current_date = datetime.datetime.now().date()
    current_date_format = current_date.strftime(r"%Y%m%d")
    snp_500_filpath = "..\snp\\" + current_date_format + ".csv"
    df_snp_500.to_csv(snp_500_filpath)

    return stocks

def FetchStocksList():
    
    live_list = get_sp500_symbols_from_financialmodelingprep()
