...
time_to_fetch_data = datetime.time(configurations["Quandl-Split"]["time_to_fetch_data"][0],configurations["Quandl-Split"]["time_to_fetch_data"][1],configurations["Quandl-Split"]["time_to_fetch_data"][2])
next_fetch_date = datetime.date(configurations["Quandl-Split"]["next_fetch_date"][0],configurations["Quandl-Split"]["next_fetch_date"][1],configurations["Quandl-Split"]["next_fetch_date"][2])

print("The Quandl-Split-Adjustment Program is Runing.......")
...
while True:
    ...
    if((today_date == next_fetch_date) and (dt.now(tz).time() >= time_to_fetch_data)):
        ...
        entries = os.listdir('Quandl-Data-Split-Applied/')
        ...
        if(bool(split_data)):
            for entry in entries:
                ...
                df = pd.read_csv("Quandl-Data-Split-Applied/" + entry)
                ...
                for stock in split_data:
                    ...
                    if(current_ratio != -1):
                        ...
                        df['OpenPrice'] = np.where(df['Ticker'] == stock, df['OpenPrice'] / current_ratio, df['OpenPrice'])
                        df['HighPrice'] = np.where(df['Ticker'] == stock, df['HighPrice'] / current_ratio, df['HighPrice'])
                        df['LowPrice'] = np.where(df['Ticker'] == stock, df['LowPrice'] / current_ratio, df['LowPrice'])
                        df['ClosePrice'] = np.where(df['Ticker'] == stock, df['ClosePrice'] / current_ratio, df['ClosePrice'])
                ...
                df.to_csv("Quandl-Data-Split-Applied/" + entry, index = False)
        else:
